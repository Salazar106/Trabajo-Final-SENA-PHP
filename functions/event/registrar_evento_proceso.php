...
$query = $bd->prepare("SELECT * FROM usuario_citas WHERE id_user = ? AND id_event = ? AND semana = ?");
...
$query = $bd->prepare("SELECT COUNT(u.id_user) as 'cantidad' FROM usuario_citas u WHERE id_user = ? AND semana = ?");
...
if(count($usuario_evento) > 0) {
    ...
    $query = $bd->prepare("INSERT INTO usuario_citas (id_user, id_event, semana, dia, hora) VALUES (?, ?, ?, ?, ?);");
